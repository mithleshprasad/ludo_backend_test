...
RoomSchema.methods.addPlayer = function (name, id) {
    ...
    if (playerCount === 0) {
        color = 'red'; // First player
    } else if (playerCount === 1) {
        color = 'green'; // Second player
    } else {
        color = COLORS[playerCount]; // Use existing color assignments for more players
    }
    ...
    this.isFull(); // Check if the room is full
};
